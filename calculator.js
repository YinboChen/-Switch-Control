...
// added or removed holdcursor for selected controlling number buttons
function selectItemNum(name){
	$("button").removeClass("holdcursor");
	$(name).addClass("holdcursor")
}
...
// this function allows user to select number of controlling keys
...
//defined i 
//give i different value
var i;
$(".selectnumkey").click(function(){
	
	if($(this).attr("id")=="1keybutton"){
		selectItemNum(this);
		i=1;
		//alert("loop start!");
		autoLoop();
		$("#number_input").val("");
	}else if($(this).attr("id")=="2keybutton"){
		selectItemNum(this);
		i=2;
		clearAll();
	}else if($(this).attr("id")=="3keybutton"){
		selectItemNum(this);
		i=3;
		clearAll();
	}else if($(this).attr("id")=="5keybutton"){
		selectItemNum(this);
		i=5;
		clearAll();
	}
})
//Using this method to clear the timer, selected cursor and clearing num_input
function clearAll(){
	clearInterval(timer1);
	$("button").removeClass("cursor");
	$("#number_input").val("");
